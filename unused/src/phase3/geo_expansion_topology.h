                                         ...
                                         uint8_t          activated_face)
{
    ...
    /* Geometry-aware: use parent's slope as the derivation pattern */
    uint64_t pattern = apex_slope(parent->master_core, activated_face);
    uint64_t child_core = apex_derive_child(parent->master_core,
                                             pattern,
                                             parent->generation + 1);

    /* Post-correction: ensure slope alignment between parent and child */
    uint64_t want = apex_slope(parent->master_core, activated_face);
    uint64_t got = apex_slope(child_core, (activated_face + 3) % 6);
    printf("[DEBUG] expansion_expand: want=0x%lx got=0x%lx\n", (unsigned long)want, (unsigned long)got);
    child_core ^= (want ^ got);
    printf("[DEBUG] expansion_expand: new_got=0x%lx\n", (unsigned long)apex_slope(child_core, (activated_face + 3) % 6));
